...
#### 2.1.1 基于 XMLHttpRequest、fetch 上报

XMLHttpRequest 和 fetch() 都是浏览器提供的 HTTP 请求 API，用于前后端数据交互，使用方式简单，可设置请求头、请求体并处理响应，支持同步和异步请求。但在高并发场景下可能阻塞主业务流程，更适用于低并发或需精细控制请求行为(如多种请求方法或依赖服务端响应)的场景。

#### 2.1.2 基于 img 标签的 src 上报

Image Beacon 是利用图片请求进行数据上报的技术，这是最经典、最可靠的前端上报方案。其原理是创建一个 Image 对象,src 指向上报接口,浏览器解析 URL后会发送一个 HTTP 请求到该接口并返回 1x1 像素的 GIF 图片,其优缺点如下:

- 无阻塞异步加载:图片请求是异步的,不会阻塞页面的其他资源加载,也不会影响页面的渲染。
- 兼容性好:所有浏览器都支持 Image Beacon,包括 IE6+。
- 支持跨域:图片天然支持跨域,无需 CORS。
- 无预检请求:与 fetch 不同,Image Beacon是一个GET请求,不会向服务器发送预检请求。
- URL 长度限制:Image Beacon 的 URL 长度受浏览器限制(不同浏览器 URL 长度限制不同),一般为 2048 个字符,超过长度限制的部分会被截断。
- GET 请求限制:只能使用 GET 方法,因此无法设置自定义请求头、无法发送 JSON 等复杂格式、数据需要 URL 编码。
- 无法获取响应:由于 Image Beacon 是一个 GET 请求,只知道请求成功,因此无法获取服务器返回的数据,例如请求耗时、响应大小等。

#### 2.1.3 基于 Beacon 异步上报
