...
由于 JS 中也提供了 typeof 操作符,所以 TS 中`typeof`有在非类型声明处和类型声明处两种用法,在非类型声明处使用`typeof`作用跟 js 中的`typeof`是相同的,用于返回表达式的类型,在类型声明处使用`typeof`表示会引用变量或属性的类型。
...
`readonly` 表示只读修饰符,一旦使用`readonly`修饰后,则不可修改。
...
- `?.`:`?.`表示可选链操作符(3.7 版本新特性),当`?.`前面的表达式为 null 或 undefined 时,TypeScript 会立即停止运行`?.`后面的表达式。
